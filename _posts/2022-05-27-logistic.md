...
* Deviance residuals are $r_{i} = sign(y_{i} - \hat{p_{i}})$

\sqrt{-2\left\{y_{i}ln(\hat{p_{i}}) + (1-y_{i}) ln(1 - \hat{p_{i}})\right\}}$, where $y_{i}$ is either 0 or 1, so if $y_{i}=0$, then $sign() = +$
